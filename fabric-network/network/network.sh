...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "Creating wholeseller Identities"
  ...
  infoln "Generating CCP files for farmer and Org2"
...
function networkUp() {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/${CONTAINER_CLI}/docker-compose-couch.yaml"
  fi
...
function networkDown() {
  ...
  COMPOSE_COUCH_FILES="-f compose/${COMPOSE_FILE_BASE} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_farmer.example.com docker_mill.example.com docker_m1.mill.example.com docker_f1.farmer.example.com docker_w1.wholeseller.example.com docker_wholeseller.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/mill/msp organizations/fabric-ca/mill/tls-cert.pem organizations/fabric-ca/mill/ca-cert.pem organizations/fabric-ca/mill/IssuerPublicKey organizations/fabric-ca/mill/IssuerRevocationPublicKey organizations/fabric-ca/mill/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/wholeseller/msp organizations/fabric-ca/wholeseller/tls-cert.pem organizations/fabric-ca/wholeseller/ca-cert.pem organizations/fabric-ca/wholeseller/IssuerPublicKey organizations/fabric-ca/wholeseller/IssuerRevocationPublicKey organizations/fabric-ca/wholeseller/fabric-ca-server.db'
...
if [ "$MODE" == "up" ]; then
 infoln "Supply chain Network Up"
  networkUp
